...
def DB_fixation_vs_mutantset(g,r,nodes,N):
    fix_prob=[]
...
# calculate the fitness of each individual for pairwise games
...
# calculate the fitness of each individual for public goods game
def calculate_fitness_pub(g,r,c,w): # g is graph, r is reword factor, c is cost
    payoff={}
    deg=g.degree()
    for d1 in g.nodes_iter():
        payoff[d1]=0
        for d2 in g.neighbors_iter(d1):
            mid1=r*c*g.node[d2]['type']/float(deg[d2]+1)
            payoff[d1]+=mid1
        payoff[d1]=payoff[d1]+(r/float(deg[d1]+1)-1)*c*g.node[d1]['type']
        payoff[d1]=1-w+w*payoff[d1]
    nx.set_node_attributes(g,'fitness',payoff)
    return g
...
# selecte the individual for death with a probability inverse proportional to the fitness
def BD_selected_death_game(g):
    nodes=g.nodes()
    totalfit=0
    for d1 in nodes:
        totalfit=totalfit+float(1)/g.node[d1]['fitness']
    rand=random.random()
    start=0
    end=float(1)/g.node[nodes[0]]['fitness']/float(totalfit)
    for i in range(len(nodes)):
        if (rand>=start) and (rand<end):
            death=nodes[i]    # birth is node
            break
        start=end
        end=end+float(1)/g.node[nodes[i+1]]['fitness']/float(totalfit)
    return death
